~~Primer logo and top bar is further from the sides than the top. THis bothers me.~~
~~Sort prompts into sections based on the type (base prompts, edit/quick-select prompts, etc)~~
~~The settings menu should be arranged like an actual primer, i.e. with the table of contents to the left.~~
~~Settings is a little overwhelming. Many fields to type into. Can we expose these when people press on the relevant settings to expand them?~~
~~The settings should have sections with table of contents, like "account", "prompts", "usage and API keys", "display" etc etc.~~
~~"Every primer is written on a key: an Anthropic API key or a Claude subscription token, your own or one someone shared with you. A subscription token comes from running claude setup-token." this thing is wrapped manually for no reason~~
~~Settings should have no horizontal bars between individual settings, not sure why those are there.~~
~~About doesn't work when inspector is open~~
~~Primer and settings bar at top comes down when you hit the top of the page when scrolling, fix~~
